...
class SceneTitle
  ...
  def tick(args)
    ...
    args.outputs.labels << [640, 540, "Tetris!", 32, 1, 0,0,0,255, 'Funstella.ttf']
    @menu.tick(args)
  end
end
...
class SceneGame
  ...
  def rotate_tetromino_left
    t = @tet.copy
    t.rot_left
    if validate_position(t) || wall_kick(t)
      @tet = t
      @fall_ticks = 0 if check_lock(@tet)
    end
  end
  ...
  def lock_tetromino
    ...
    for y in 0..3
      for x in 0..3
        if @tet.blocks[y*4 + x]
          @stage[@tet.x+x, @tet.y+y] = @tet.type if @tet.y+y >= 0 # Don't overflow top
        end
      end
    end
  ...
  def next_tetromino
    @tet = @queue.pop
    @queue.unshift(Tetromino.new(3, -3, @bag.pop))
    @tet.x = 3
    @tet.y = -2
    @just_held = false
  ...
  def tick_paused(args)
    args.outputs.labels << [640, 540, "PAUSED", 32, 1, 0,0,0,255, 'Funstella.ttf']
  end
  def tick_gameover(args)
    args.outputs.labels << [640, 540, "GAME OVER!", 32, 1, 0,0,0,255, 'Funstella.ttf']
  end
    ...
    for y in 0..STAGE_H-1
      for x in 0..STAGE_W-1
        if @stage[x][y] != ' '
          ...
          args.outputs.sprites << [STAGE_X+x*32, STAGE_Y-y*32, 32, 32, 'sprites/block.png', 0, 255] + color
        end
      end
    end
    # Current tetromino
    @tet.draw(STAGE_X, STAGE_Y+STAGE_H*32-32, args) if @tet
    # Phantom tetromino
...
class SceneScore
  ...
  def tick(args)
    args.outputs.labels << [640, 540, "IT'S DA HIGH SCORES!!!", 32, 1, 0,0,0,255, 'Funstella.ttf']
  end
end
...
class SceneSettings
  ...
  def tick(args)
    args.outputs.labels << [640, 540, "IT'S DA SETTINGS!!!", 32, 1, 0,0,0,255, 'Funstella.ttf']
  end
end
